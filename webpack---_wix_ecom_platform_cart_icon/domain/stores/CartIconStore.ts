...
export class CartIconStore {
  ...
  private wasInvalidateCacheCalled: boolean = false;
  ...
  public updateCart(cart: ICart): void {
    ...
    void this.cartPromise?.then(({widgetSettings}) => {
      const shouldUseMemberIdAsCacheKey = this.siteStore.experiments.enabled(SPECS.ShouldUseMemberIdAsCacheKey);
      const cartData: IDataResponse = {cartSummary: cart, widgetSettings};
      if (this.siteStore.experiments.enabled(SPECS.UseCartCache) && !this.wasInvalidateCacheCalled) {
  ...
  public invalidateCache(): void {
    if (this.siteStore.experiments.enabled(SPECS.UseCartCache)) {
      void this.cartCacheService.clear();
    }
  }
  ...
  public registerEvents(): void {
    ...
    this.pubSubManager.subscribe('Cart.Invalidated', () => {
      ...
      this.wasInvalidateCacheCalled = true;
    });
    ...
    return Promise.all([this.getAppSettingsData(), this.siteStore.getSectionUrl(PageMap.CART)])
      .then(([serverResponse, cartLink]) => {
        ...
        const props = {
          ...
          shouldFixIcon4Appearance: this.siteStore.experiments.enabled(SPECS.FixIcon4Appearance),
          shouldNotShowCartIconInSsrStudio: this.siteStore.experiments.enabled(SPECS.ShouldNotShowCartIconInSsrStudio),
          shouldChangeOverflowIconContainer: this.siteStore.experiments.enabled(SPECS.ChangeOverflowIconContainer),
        } as ICtrlProps;
        this.setProps(props);
  ...
  public async getData(): Promise<IDataResponse> {
    ...
    if (this.siteStore.experiments.enabled(SPECS.UseCartCache)) {
      ...
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_WITHOUT_CACHE).start();
      this.cartPromise = this.loadCartFromServer().then((cartData) => {
        ...
        this.panoramaClient?.transaction(FedopsInteraction.GET_CART_WITHOUT_CACHE).finish();
        return cartData;
      });
    }
    return this.cartPromise;
  ...
  private async getCartFromCacheOrServer(): Promise<IDataResponse> {
    try {
      ...
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_DATA).start();
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_FROM_CACHE).start();
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_FROM_CACHE_UNDEFINED).start();
      ...
      if (cartDataFromCache) {
        ...
        this.panoramaClient?.transaction(FedopsInteraction.GET_CART_DATA).finish();
        this.panoramaClient?.transaction(FedopsInteraction.GET_CART_FROM_CACHE).finish();
      ...
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_FROM_CACHE_UNDEFINED).finish();
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_FROM_SERVER).start();
      const cartData = await this.loadCartFromServer();
      this.fedops.interactionEnded(FedopsInteraction.GET_CART_FROM_SERVER);
      this.fedops.interactionEnded(FedopsInteraction.GET_CART_DATA);
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_FROM_SERVER).finish();
      this.panoramaClient?.transaction(FedopsInteraction.GET_CART_DATA).finish();
      if (!this.wasInvalidateCacheCalled) {
        void this.cartCacheService.set(this.getCachedMemberId(), cartData, shouldUseMemberIdAsCacheKey);
      }
      return cartData;
    } catch (e) {
