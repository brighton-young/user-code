  ...
  return function serialize(json = {}, converterType) {
    ...
    function transformSchema(schema, payload) {
      ...
      Object.entries(payload).forEach(([key, val]) => {
        ...
        const { schemaName, schemaType } = (0, _.parseLeanSchemaRef)(
          renderedSchemaName
        );
        const isMap = schemaType === "Map";
        const isRepeatable =
          ((_getConverter = getConverter(schemaName)) == null ||
          _getConverter.checkRepetable == null
            ? void 0
            : _getConverter.checkRepetable(val)) ?? Array.isArray(val);
        let parsedValue;
        if (isRepeatable) {
    ...
    function getConverter(name) {
      ...
      return (_converterSets$name = converterSets[name]) == null
        ? void 0
        : _converterSets$name[converterType];
    }
