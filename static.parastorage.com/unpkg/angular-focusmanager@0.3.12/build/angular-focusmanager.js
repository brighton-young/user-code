...
(function (exports, global) {
  var consts = {
    ...
    GROUP_ELEMENTS_STRICT:
      '[{focusParentId}="{groupId}"][focus-index]:not([disabled]):not(.focus-ignore):not(.disabled)',
    GROUP_ELEMENTS:
      '[{focusParentId}="{groupId}"]:not([disabled]):not(.focus-ignore):not(.disabled)',
  };
  ...
  module.directive("focusAutofocus", [
    "focusManager",
    "focusQuery",
    function (focusManager, focusQuery) {
      var focusEls = [];
      var focusElsCount = 0;
      var unwatchChanges;
      var timer;
      ...
      return {
        ...
        link: function (scope, element, attr) {
          var el = element[0];
          var focusIndex = !attr.focusAutofocus
            ? 0
            : parseInt(attr.focusAutofocus, 10);
  ...
  module.directive("focusGroup", [
    "focusManager",
    "focusQuery",
    "focusDispatcher",
    "focusKeyboard",
    function (focusManager, focusQuery, focusDispatcher, focusKeyboard) {
      var groupId = 1,
        elementId = 1,
      ...
      function linker(scope, element, attr) {
        ...
        function init() {
          ...
          if (!focusQuery.getParentGroupId(el)) {
            ...
            focusInOff = dispatcher.on(
              "focusin",
              utils.debounce(function (evt) {
            ...
            focusEnabledOff = dispatcher.on("enabled", function (evt) {
              ...
              if (
                ...
                focusQuery.contains(el, document.activeElement)
              ) {
  ...
  module.directive("focusHighlight", [
    "focusManager",
  ...
  module.directive("focusShortcut", [
    "focusManager",
    function (focusManager) {
      return {
        link: function (scope, element, attrs) {
  ...
  module.service("focusManager", [
    "focusQuery",
    "focusDispatcher",
    function (focusQuery, focusDispatcher) {
      var scope = this,
        dispatcher = focusDispatcher();
      function focus(el) {
      ...
      function getActiveElement() {
        return focusQuery.getElement(
          focusQuery.getElementId(scope.activeElement)
        );
      }
  ...
  module.service("focusQuery", function () {
    ...
    function canReceiveFocus(el) {
      ...
      var isSelectable = new RegExp(el.nodeName.toUpperCase()).test(
        consts.SELECTABLE
      );
      if (!isSelectable) {
      ...
      if (!isSelectable) {
        isSelectable =
          el.hasAttribute(consts.TAB_INDEX) &&
          el.getAttribute(consts.TAB_INDEX) > -1;
      }
      if (isSelectable) {
    ...
    function getGroupsWithoutParentGroup(el) {
      ...
      var q =
        "[" +
        consts.FOCUS_GROUP_ID +
        "]:not([" +
        consts.FOCUS_PARENT_GROUP_ID +
        "])";
    ...
    function sortByTabIndex(a, b) {
      var aTabIndex =
        parseInt(a.getAttribute(consts.FOCUS_INDEX), 10) ||
        Number.POSITIVE_INFINITY;
      var bTabIndex =
        parseInt(b.getAttribute(consts.FOCUS_INDEX), 10) ||
        Number.POSITIVE_INFINITY;
    ...
    function sortByGroupIndex(a, b) {
      var aGroupIndex =
        parseInt(a.getAttribute(consts.FOCUS_GROUP_INDEX), 10) ||
        Number.POSITIVE_INFINITY;
      var bGroupIndex =
        parseInt(b.getAttribute(consts.FOCUS_GROUP_INDEX), 10) ||
        Number.POSITIVE_INFINITY;
