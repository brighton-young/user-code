...
var AmbassadorHTTPClient = /** @class */ (function () {
    ...
    this.handleFetchErrors = function (err) {
      ...
      if (fetchError.message) {
        ...
        return tryParseFetchResponse(fetchResponse).then(function (message) {
          var _a;
          var requestId =
            (_a = fetchResponse.headers) === null || _a === void 0
              ? void 0
              : _a.get(REQUEST_ID_HEADER);
          throw new AmbassadorHTTPError(
  ...
  AmbassadorHTTPClient.prototype.assignAxiosClient = function () {
    ...
    if (config.wixHttpClientInstance) {
      ...
      var redaxios =
        require("redaxios").default /* webpack */ ||
        require("redaxios"); /* nodejs */
      this.useRedaxios = true;
      this._axiosClient = redaxios.create({ responseType: "json" });
    }
  };
  AmbassadorHTTPClient.prototype.fixProtoIgnoredUrlPart = function (
    originalPath
  ) {
    var originalPathParts = originalPath.split(this.ignoredProtoUrlPart);
    if (
      originalPathParts[0] === "" &&
      (originalPathParts[1].startsWith("/") || originalPathParts[1] === "")
    ) {
      return originalPath.substr(
        this.ignoredProtoUrlPart.length,
        originalPath.length
      );
    } else {
      return originalPath;
  ...
  AmbassadorHTTPClient.prototype.invoke = function (
    method,
    path,
    payload,
    headers
    ...
    var normalizedPath = this.removeLeadingSlash(
      this.fixProtoIgnoredUrlPart(path)
    );
    var paramsInPath = this.getParamsFromPath(normalizedPath);
    var encodedParamsPath = paramsInPath.reduce(function (path, nextParam) {
      return path.replace(
        "{" + nextParam + "}",
        encodeURIComponent(_this.getValueFromPayload(payload, nextParam))
      );
    }, normalizedPath);
    if (this.fetchClient) {
      ...
      headers = __assign(
        __assign({}, this.createWixHeaders({ csrf: this.useRedaxios })),
        headers
      );
      var requestConfig =
  ...
  AmbassadorHTTPClient.prototype.runHttpRequest = function (
    method,
    fixedPath,
    payload,
    requestConfig
    ...
    switch (method) {
      case "GET":
        return this._axiosClient
          .get(
            fixedPath + "?" + this.payloadToQueryString(payload),
            requestConfig
          )
          .then(function (res) {
            return res.data;
      ...
      case "POST":
        return this._axiosClient
          .post(fixedPath, payload, requestConfig)
          .then(function (res) {
            return res.data;
          })
          .catch(this.handleErrors);
      case "PUT":
        return this._axiosClient
          .put(fixedPath, payload, requestConfig)
          .then(function (res) {
            return res.data;
          })
          .catch(this.handleErrors);
      case "PATCH":
        return this._axiosClient
          .patch(fixedPath, payload, requestConfig)
          .then(function (res) {
            return res.data;
          })
          .catch(this.handleErrors);
  ...
  AmbassadorHTTPClient.prototype.fetchHttpRequest = function (
    method,
    fixedPath,
    payload,
    headers
  ) {
    var fullPathUrl = combineURLs(
  ...
  AmbassadorHTTPClient.prototype.mergeHeadersWithContentType = function (
    headers
  ) {
    var appJsonHeaders = {
    ...
    return Object.keys(payload)
      ...
      .map(function (key) {
        if (payload[key] instanceof Array) {
          ...
          var flattenedObject = Object.keys(payload[key]).reduce(function (
            obj,
            innerKey
          ) {
            var _a;
            return __assign(
              __assign({}, obj),
              ((_a = {}),
              (_a[key + "." + innerKey] = payload[key][innerKey]),
              _a)
            );
          },
          {});
  ...
  AmbassadorHTTPClient.prototype.getValueFromPayload = function (
    payload,
    pathToField
  ) {
    ...
    return pathParts.reduce(function (payload, nextPathPart) {
      return (
        payload[nextPathPart] ||
        payload[(0, utils_1.snakeToCamelCase)(nextPathPart)]
      );
    }, payload);
  };
