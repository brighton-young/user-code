...
export function createRegistry({ host }) {
  return async ({ getSentryClient, isExperimentOpen, ...params }) => {
    ...
    const resourceLoader = runAndReport.sync(
      () =>
    ...
    if (registryOverrideLibrary) {
      const registry = await runAndReport.async(
        () =>
          loadExternalRegistry({
            resourceLoader,
            host,
    ...
    const context = await runAndReport.async(
      async () =>
        loadContext({
          host,
          libraries,
    ...
    if (!options.useLazyLoadersInit) {
      await runAndReport.async(
        async () => createComponentLoaders(context.manifests),
      ...
      getHostBundleAssets() {
        return flat(
          context.manifests.map(({ manifest }) => {
            const model = runAndReport.sync(
              () => manifest.getHostBundleModel(),
              {
                methodName: "manifest.getHostBundleModel",
                apiFunctionName: "getHostBundleAssets",
              }
            );
            return runAndReport.sync(
              () => getComponentAssets({ resource: model }),
              {
      ...
      async ensureComponentLoadersAreCreated() {
        if (!areComponentsLoadersCreated) {
          if (!pendingPromises.componentsLoaders) {
            pendingPromises.componentsLoaders = runAndReport.sync(
              () => createComponentLoaders(context.manifests),
              {
                methodName: "createComponentLoaders",
        ...
        if (!isContextFulfilled && context.loadManifests) {
          if (!pendingPromises.librariesManifests) {
            pendingPromises.librariesManifests = new Promise(
              async (resolve) => {
                const manifests = await runAndReport.async(
                  async () => context.loadManifests(),
                  {
                ...
                await runAndReport.async(
                  () => createComponentLoaders(manifests),
                  {
                    methodName: "createComponentLoaders",
                    apiFunctionName: "ensureManifestsAreLoaded",
                  }
