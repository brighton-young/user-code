...
const componentsLoaderFactory: ComponentsLoaderFactory = (
	...
	hydrateOnViewport.tpaWidgetNative = !viewerModel.experiments['specs.thunderbolt.ooi_lazy_load_components']

	if (viewerModel.experiments['specs.thunderbolt.viewport_hydration_extended_react_18']) {
		Object.assign(hydrateOnViewport, {
			Section: true,
			AppWidget: true,
			ClassicSection: true,
		})
	...
	const loadComponent = async (compType: string) => {
		...
		if (
			...
			// Keep this until we have a unified solution components & OOIs
			// const originalComponent = componentsRegistry[compType]
			// const deferredComponentLoaderFactory = lazyComponentLoaderFactory({
			// 	componentResolver: () => Promise.resolve(originalComponent),
			// })
			// const comp = WithHydrateWrapper({
			// 	debugRendering,
			// 	deferredComponentLoaderFactory,
			// 	onUnmount: (componentObserver: LazyComponentLoaderResponse) =>
			// 		componentObserver.clearComponentListeners(),
			// 	Comp: originalComponent /* Comp is for ssr */,
			// })
			const comp = WithHydrateWrapper({ Comp: componentsRegistry[compType] })
