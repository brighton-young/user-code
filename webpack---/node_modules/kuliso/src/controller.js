...
function centerToTargetFactory(target, root) {
  ...
  return {
    x(x1) {
      ...
      const xDuration =
        (isXStartFarthest ? layerCenterX : root.width - layerCenterX) * 2;
    ...
    y(y1) {
      ...
      const yDuration =
        (isYStartFarthest ? layerCenterY : root.height - layerCenterY) * 2;
...
export function getController(config) {
  ...
  config.scenes.forEach((scene) => {
    if (scene.target && scene.centeredToTarget) {
      scene.transform = centerToTargetFactory(
        getRect(scene.target),
        config.rect
      );
    ...
    for (let scene of config.scenes) {
      if (!scene.disabled) {
        ...
        const x = +clamp(
          0,
          1,
          scene.transform?.x(progress.x) || progress.x / config.rect.width
        ).toPrecision(4);
        const y = +clamp(
          0,
          1,
          scene.transform?.y(progress.y) || progress.y / config.rect.height
        ).toPrecision(4);
