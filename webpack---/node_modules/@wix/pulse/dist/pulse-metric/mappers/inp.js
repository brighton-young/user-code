...
function calculateIsINPeventDuringNavigation(inpStartTime, navigationParams) {
  ...
  const { start: navigationStart, end: navigationEnd } =
    navigationParams.lastNavigationTimings;
...
export const toInpMetric = (metric, getHtmlElementMetadata) => {
  ...
  const htmlElementMetadata = getHtmlElementMetadata(
    getEventTarget(metric) || null
  );
  const navigationParams = htmlElementMetadata.navigationParams;
  const inpIsDuringNavigation = calculateIsINPeventDuringNavigation(
    interactionTime,
    navigationParams
  );
  return {
