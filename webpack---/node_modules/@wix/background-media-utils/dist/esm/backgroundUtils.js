  ...
  return (
    (typeof window !== "undefined" &&
      ((_serviceTopology = window.serviceTopology) == null
        ? void 0
        : _serviceTopology.staticMediaUrl)) ??
    ""
  );
};
...
const parseVideoQualities = (fileOutput) => {
  ...
  const storyboard =
    (_fileOutput$storyboar = fileOutput.storyboard) == null
      ? void 0
      : _fileOutput$storyboar.find((_ref2) => {
          let { format } = _ref2;
          return format === "mp4";
        });
  const qualities = mp4Videos.map((_ref3) => {
    let { width, height, quality, url } = _ref3;
    return {
      width,
      height,
      quality,
...
export function getImageBackgroundObject(fileInfo, info) {
  ...
  return {
    ...
    hasAnimation: !!(
      (_getObjectValueByKey = getObjectValueByKey(fileInfo, "file_input")) !=
        null && _getObjectValueByKey.animated
    ),
  };
}
...
export function getMediaType(background) {
  ...
  return (
    (background == null || (_background$mediaRef = background.mediaRef) == null
      ? void 0
      : _background$mediaRef.type) ?? MEDIA_TYPES.COLOR
  );
}
