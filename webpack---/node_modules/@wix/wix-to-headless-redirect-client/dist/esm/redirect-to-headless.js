...
export const createWixToHeadlessUrl = (_ref2) => {
  ...
  return `${wixBaseUrl}/_serverless/wix-to-headless-redirect/redirect?targetUrl=${encodeURIComponent(
    targetBaseUrl
  )}&redirectLogicalName=${
    navigationParams.logicalName
  }&params=${JSURL.stringify(
    navigationParams.params ?? {}
  )}&clientId=${clientId}&navigationData=${JSURL.stringify(
    navigationData ?? {}
  )}`;
};
export const getHeadlessUrl = (_ref3) => {
  ...
  const redirectUrls =
    JSURL.tryParse(query == null ? void 0 : query.headlessExternalUrls, {}) ??
    {};
