...
const { sanitize } = require("@wix/wnp-log-sanitizer");
...
class DebugLogger {
  constructor(name, requestData = {}, { skipEnablingErrors, customData } = {}) {
    ...
    if (
      !skipEnablingErrors &&
      process.env.DEBUG_ENV_ONLY !== "true" &&
      !isNamespaceEnabled(errorNamespace)
    ) {
    ...
    ["info", "debug", "error", "trace", "warn"].forEach((level) => {
      ...
      this[`_${level}`] = logWith(
        cachedLogger(levelNamespace),
        this._requestData,
        this._customData,
        levelNamespace,
        level
  ...
  withData(data) {
    return new DebugLogger(this._name, this._requestData, {
...
function logWith(logger, requestData, customData, namespace) {
  ...
  return (argsArray) => {
    ...
    const argsWithData = mergeDataItems(argsArray, {
      ...errorDetailsData,
      ...customData,
    });
    const text = util.format.apply(
      util,
      toKeyValueStringPairs(requestData)
        .concat(argsWithData)
        .map((el) => debug.coerce(el))
    );
...
function getErrorArgAddedData(argsArray) {
  ...
  return (
    errorArg &&
    errorArg.details && { _errorDetails: buildErrorDetails(errorArg) }
  );
}
