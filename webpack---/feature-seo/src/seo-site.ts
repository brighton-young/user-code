...
const shouldRenderDom = (featureState: IFeatureState<SeoFeatureState>, siteName?: string): boolean => {
	...
	const isPageTitleMissingPageName = !isInSSR && siteName === document?.title
	const isClientReady = !isInSSR && isPageMounted && isAfterNavigation
	return isClientReady || isClientFallback || noTitleExistsOnClient || isPageTitleMissingPageName
}
...
export const SeoSite = withDependencies(
	...
	(
		...
		const renderSEODom = async () => {
			if (shouldRenderDom(featureState, siteLevelSeoData.context.siteName)) {
