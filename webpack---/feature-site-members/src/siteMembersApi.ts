	...
	const api = {
		...
		async sendForgotPasswordMail(email: string) {
			...
			const body = experiments['specs.thunderbolt.iamResetPasswordFlow']
				? JSON.stringify({ language: userLanguage, email, redirect })
		...
		async logout(redirectToUrl?: string) {
			...
			if (experiments['specs.thunderbolt.shouldRedirectOnLogout']) {
				const headlessLogoutRedirectUrl = await api.getLogoutUrl()

				if (headlessLogoutRedirectUrl) {
					const relativeUrl = `./${headlessLogoutRedirectUrl.replace(/^\//, '')}`
		...
		getLogoutUrl() {
			if (clientId && experiments['specs.thunderbolt.shouldFetchLogoutUrlByClientId']) {
