...
export const getActiveAnchor = (anchors: Array<Anchor>, offset: number, browserWindow: BrowserWindow): ActiveAnchor => {
	...
	/*
	 * There could be two items in anchors have equal `(anchor.top - offset)` value,
	 * in this case we should take the first one (which is anchor, not section).
	 */
	if (anchors[nextActiveAnchorIndex - 1]?.top === anchors[nextActiveAnchorIndex - 2]?.top) {
		return convertAnchorToActiveAnchor(anchors[nextActiveAnchorIndex - 2])
	}
